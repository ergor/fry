...
use crate::generator2::{Delta, is_square_empty, is_square_en_passant, is_square_enemy, MoveType, push_if_legal};
...
pub(super) fn generate_pawn(board: &Board, piece_pos: Index2D, color: Color) -> Vec<Delta> {
    ...
    let pawn_capture = |vector: Vector2D| {
        if let Some(landing_pos) = piece_pos + vector {
            ...
            } else if is_square_en_passant(board, landing_pos) {
                push_if_legal(&mut moves, board, MoveType::EnPassant, piece_pos, landing_pos);
            }
        } // else the move landed outside of board.
    };
    ...
    if (color == Color::White && piece_pos.y == 1) || (color == Color::Black && piece_pos.y == 6) {
